...
#
# Once allocated:
#   bash ops/slurm/fep/smoke_neq_em.sh
#
set -euo pipefail
...
echo "  STAGE=extract bash ops/slurm/fep/smoke_neq_task.sh"
echo "  STAGE=switch SNAPSHOT=0 bash ops/slurm/fep/smoke_neq_task.sh"
exec "${CMD[@]}"
