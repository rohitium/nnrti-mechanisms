...
#
# Usage:
#   bash ops/slurm/fep/build_p0_systems.sh
#   REPLICATES=1 PHASES=holo bash ops/slurm/fep/build_p0_systems.sh
#   FORCE=1 bash ops/slurm/fep/build_p0_systems.sh
#
set -euo pipefail
...
# GROMACS (Sherlock)
if [[ -f ops/slurm/cluster/load_gromacs_module.sh ]]; then
...
if ! command -v gmx >/dev/null 2>&1; then
    echo "ERROR: gmx not found. On Sherlock: source ops/slurm/cluster/load_gromacs_module.sh" >&2
    exit 1
fi
if ! command -v pmx >/dev/null 2>&1; then
    echo "ERROR: pmx not found. On Sherlock: source ~/.venvs/pmx/bin/activate" >&2
    echo "       Or install: bash ops/slurm/cluster/setup_pmx_env.sh" >&2
    exit 1
fi
if [[ -z "${GMXLIB:-}" ]]; then
