...
#   NEQ_SNAPSHOTS=100 REPLICATES=3 bash ops/slurm/fep/prepare_p0_neq.sh --rebuild-panel-only
#   DEPENDENCY=afterok:<extract_job> STAGE=switch bash ops/slurm/fep/submit_p0_neq.sh
#
set -euo pipefail
...
echo "  NEQ_SNAPSHOTS=${NEQ_SNAPSHOTS} REPLICATES=${REPLICATES} bash ops/slurm/fep/prepare_p0_neq.sh --rebuild-panel-only"
echo "  DEPENDENCY=afterok:<extract_job_id> STAGE=switch bash ops/slurm/fep/submit_p0_neq.sh"
