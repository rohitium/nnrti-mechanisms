...
| --- | --- | --- |
| **GPU MD** | `source ops/slurm/cluster/load_gromacs_module.sh` | Sets `GMX_MDRUN=gmx_cuda` |
...
| --- | --- | --- |
| **pmx** | `bash ops/slurm/fep/setup_pmx_env.sh` — [deGrootLab/pmx](https://github.com/deGrootLab/pmx) **`develop`**; **not** `pip install pmx` | **Mac** (prep/analysis); optional on Sherlock login |
