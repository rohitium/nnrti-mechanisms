...
|---|---|---|
| `future feature annotations is not defined` | ran with login-node Python 3.6 | `source ops/slurm/cluster/activate_pmx_env.sh` (§0) |
...
STAGE=equil bash ops/slurm/fep/submit_p0_neq.sh
...
EQUIL=$(STAGE=equil bash ops/slurm/fep/submit_p0_neq.sh | tail -1);        echo "equil=$EQUIL"

# 3) re-chain extract (CPU) then switch (GPU)
EXTRACT=$(STAGE=extract DEPENDENCY=afterok:$EQUIL   bash ops/slurm/fep/submit_p0_neq.sh | tail -1); echo "extract=$EXTRACT"
SWITCH=$(STAGE=switch   DEPENDENCY=afterok:$EXTRACT bash ops/slurm/fep/submit_p0_neq.sh | tail -1); echo "switch=$SWITCH"
...
STAGE=em bash ops/slurm/fep/submit_p0_neq.sh                 # CPU only
...
EQUIL=$(STAGE=equil   bash ops/slurm/fep/submit_p0_neq.sh | tail -1)
EXTRACT=$(STAGE=extract DEPENDENCY=afterok:$EQUIL   bash ops/slurm/fep/submit_p0_neq.sh | tail -1)
SWITCH=$(STAGE=switch  DEPENDENCY=afterok:$EXTRACT bash ops/slurm/fep/submit_p0_neq.sh | tail -1)
```
...
EQUIL=$(TASK_ID_FILE=$PWD/results/analysis/fep_pmx/neq_equil_<batch>_task_ids.txt \
        STAGE=equil bash ops/slurm/fep/submit_p0_neq.sh | tail -1)
EXTRACT=$(TASK_ID_FILE=$PWD/results/analysis/fep_pmx/neq_extract_<batch>_task_ids.txt \
        STAGE=extract DEPENDENCY=afterok:$EQUIL bash ops/slurm/fep/submit_p0_neq.sh | tail -1)
...
**Run the first analysis pass ON SHERLOCK, not the Mac.** The rsync (`ops/sync/rsync_fep_pmx.sh`)
...
LEGS="$LEGS" bash ops/slurm/fep/prepare_p0_hybrids.sh                       # pmx hybrids, reps 1-3
REPLICATES="1 2 3" LEGS="$LEGS" bash ops/slurm/fep/build_p0_systems.sh      # gmx solvate+ionize
