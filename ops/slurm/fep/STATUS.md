...
LEGS="<leg_id>" REPLICATES="1-3" bash ops/slurm/fep/prepare_p0_hybrids.sh
source ops/slurm/cluster/load_gromacs_module.sh && source ops/slurm/cluster/activate_pmx_env.sh  # GMXLIB must be .../mutff
LEGS="<leg_id>" REPLICATES="1-3" bash ops/slurm/fep/build_p0_systems.sh
python -m nnrti.fep.prepare_neq --legs <leg_id ...> --replicates 3 --n-snapshots 100 --force
bash ops/slurm/fep/submit_p0_neq_pipeline.sh   # em→equil→extract→switch, afterok, ≤3-leg batches
```
