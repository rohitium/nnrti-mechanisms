...
#
# Usage:
#   bash ops/slurm/cluster/connect.sh
#   bash ops/slurm/cluster/connect.sh check    # connect if needed, then module probe
#
...
case "${1:-}" in
  check|modules|probe)
    ...
    ;;
  "")
    echo "Use: bash ops/slurm/cluster/remote.sh '<command>'"
    ;;
  *)
