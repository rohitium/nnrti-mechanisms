...
#
# Usage:
#   bash ops/slurm/cluster/remote.sh 'module avail gromacs'
#   bash ops/slurm/cluster/remote.sh hostname
...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash ops/slurm/cluster/remote.sh '<remote command>'" >&2
  exit 1
fi

if ! ssh -S "${SSH_CTL}" -O check "${REMOTE_HOST}" 2>/dev/null; then
  echo "No ControlMaster — run: bash ops/slurm/cluster/connect.sh" >&2
  exit 1
fi
