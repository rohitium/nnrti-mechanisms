...
#
# Usage:
#   SHERLOCK_USER=rsatija bash ops/sync/rsync_fep_pmx.sh pull   # Sherlock -> local (default)
#   SHERLOCK_USER=rsatija bash ops/sync/rsync_fep_pmx.sh push   # local -> Sherlock
#   DRY=1 SHERLOCK_USER=rsatija bash ops/sync/rsync_fep_pmx.sh pull   # preview only
...
if [[ "${DIRECTION}" != "pull" && "${DIRECTION}" != "push" ]]; then
  echo "Usage: SHERLOCK_USER=<user> bash ops/sync/rsync_fep_pmx.sh pull|push" >&2
  exit 1
fi
