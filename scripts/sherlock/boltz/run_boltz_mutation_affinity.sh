...
#   bash scripts/sherlock/boltz/run_boltz_mutation_affinity.sh WT
#
# Optional env vars:
...
if [ -z "${MUTATION}" ]; then
    echo "Usage: bash scripts/sherlock/boltz/run_boltz_mutation_affinity.sh <MUTATION|WT>" >&2
    exit 2
fi
...
"${BOLTZ_PYTHON}" scripts/sherlock/boltz/make_boltz_wt_input.py \
