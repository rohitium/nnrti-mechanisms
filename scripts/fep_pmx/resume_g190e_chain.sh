...
#
# USAGE
#   bash scripts/fep_pmx/resume_g190e_chain.sh            # audit + submit
#   AUDIT_ONLY=1 bash scripts/fep_pmx/resume_g190e_chain.sh
#
...
echo "--- audit ---"
AUDIT="$(python3 scripts/fep_pmx/audit_neq_panel.py --manifest "$MANIFEST" | grep '===')"
echo "$AUDIT"
echo ""
...
EXTRACT=$(TASK_ID_FILE=$PWD/results/analysis/fep_pmx/neq_extract_g190e_e20_task_ids.txt \
          STAGE=extract bash scripts/fep_pmx/submit_p0_neq.sh | tail -1)
