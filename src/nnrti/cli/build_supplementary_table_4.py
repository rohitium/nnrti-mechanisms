...
one row per genotype and replicate, from a single pass over three sources:
  ...
  results/analysis/pocket_volume/tables/pocket_volume_per_rep.csv
...
DEFINITIONS = [
    ...
    ("Provenance",
     "Generated by src/nnrti/cli/build_supplementary_table_4.py from "
...
def build(root: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    volume = pd.read_csv(root / "results/analysis/pocket_volume/tables/pocket_volume_per_rep.csv")
