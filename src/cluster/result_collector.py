...
def summarize_rmsd_convergence(rmsd_df: pd.DataFrame) -> pd.DataFrame:
    if rmsd_df.empty:
        return pd.DataFrame()
    rows: list[dict] = []
    for (mutation, replicate), grp in rmsd_df.groupby(["mutation", "replicate"]):
        g = grp.sort_values("frame_index")
        values = g["ca_rmsd_angstrom"].to_numpy(dtype=float)
        if values.size < 4:
            continue
        mid = values.size // 2
        first = values[:mid]
        ...
        rows.append(
            {
                ...
                "rmsd_first_half_mean": float(np.nanmean(first)),
                "rmsd_second_half_mean": float(np.nanmean(second)),
                "rmsd_delta_second_minus_first": float(np.nanmean(second) - np.nanmean(first)),
...
def summarize_com_distance_convergence(com_df: pd.DataFrame) -> pd.DataFrame:
    if com_df.empty:
        return pd.DataFrame()
    rows: list[dict] = []
    for (mutation, replicate), grp in com_df.groupby(["mutation", "replicate"]):
        g = grp.sort_values("frame_index")
        values = g["com_distance_angstrom"].to_numpy(dtype=float)
        if values.size < 4:
            continue
        mid = values.size // 2
        first = values[:mid]
        ...
        rows.append(
            {
                "mutation": mutation,
                "replicate": int(replicate),
                "com_first_half_mean_angstrom": float(np.nanmean(first)),
                "com_second_half_mean_angstrom": float(np.nanmean(second)),
                "com_delta_second_minus_first_angstrom": float(np.nanmean(second) - np.nanmean(first)),
                "n_points": int(values.size),
            }
        )
    return pd.DataFrame(rows)
...
def compute_structural_metrics(
    ...
    from ..analysis_metrics import compute_ensemble_metrics
...
def compute_mmgbsa_metrics(
    ...
    from ..openmm.mmgbsa import compute_mmgbsa_from_trajectory
...
def summarize_ddg_by_mutation(ddg_df: pd.DataFrame) -> pd.DataFrame:
    if ddg_df.empty:
        return pd.DataFrame()
    out = (
        ddg_df.groupby(["structure", "mutation", "safe_label", "fold_reduction"], as_index=False)
        .agg(
            ddg_mean=("ddg", "mean"),
            ddg_std=("ddg", "std"),
            ddg_sem=("ddg", lambda x: x.std(ddof=1) / np.sqrt(len(x)) if len(x) > 1 else 0.0),
            binding_dg_mean=("binding_dg", "mean"),
            binding_dg_std=("binding_dg", "std"),
            binding_dg_sem=("binding_dg", lambda x: x.std(ddof=1) / np.sqrt(len(x)) if len(x) > 1 else 0.0),
            n_replicates=("replicate", "nunique"),
        )
        .sort_values("ddg_mean", ascending=False)
...
def _mutation_type(mutation: str) -> str:
    if mutation == "WT":
        return "WT"
    n = len([x for x in str(mutation).split("+") if x.strip()])
    if n <= 1:
        return "single"
    if n == 2:
        return "double"
...
def make_table1_like_energy_components(ddg_df: pd.DataFrame) -> pd.DataFrame:
    if ddg_df.empty:
        return pd.DataFrame()

    work = ddg_df.copy()
    work["mutation_type"] = work["mutation"].map(_mutation_type)
    cols = [
        "binding_dg",
        "binding_dg_vdw",
    ...
    out = work.groupby(["structure", "mutation", "mutation_type", "safe_label"], as_index=False).agg(**agg_spec)
    n = out["n_replicates"].clip(lower=1).to_numpy(dtype=float)
    for col in list(out.columns):
        if col.endswith("_std"):
            out[col.replace("_std", "_sem")] = out[col] / np.sqrt(n)

    order = {"WT": 0, "single": 1, "double": 2, "triple": 3, "multi": 4}
    out["_ord"] = out["mutation_type"].map(order).fillna(9)
    out = out.sort_values(["_ord", "mutation"]).drop(columns=["_ord"]).reset_index(drop=True)
    return out


def build_mutation_position_summary(ddg_df: pd.DataFrame) -> pd.DataFrame:
    if ddg_df.empty:
        return pd.DataFrame()

    mut_df = ddg_df[ddg_df["mutation"] != "WT"].drop_duplicates(subset=["mutation", "fold_reduction"]).copy()
    if mut_df.empty:
        return pd.DataFrame()
    ...
    for _, row in mut_df.iterrows():
        ...
        fold = float(row.get("fold_reduction", np.nan))
        for part in [p for p in mutation.split("+") if p.strip()]:
            ...
            rows.append(
                {
                    ...
                    "position": pos,
                    "fold_reduction": fold,
                    "mean_log10_fold_plus1": np.log10(fold + 1.0) if np.isfinite(fold) else np.nan,
                }
            )
    ...
    return (
        detail.groupby("position", as_index=False)
        .agg(
            n_mutations=("mutation", "nunique"),
            mean_fold_reduction=("fold_reduction", "mean"),
            median_fold_reduction=("fold_reduction", "median"),
            mean_log10_fold_plus1=("mean_log10_fold_plus1", "mean"),
        )
...
def run_result_collection(
    ...
    if not rmsd_df.empty:
        ...
        summarize_rmsd_convergence(rmsd_df).to_csv(output_dir / "rmsd_convergence_summary.csv", index=False)
    ...
    if not com_df.empty:
        ...
        summarize_com_distance_convergence(com_df).to_csv(
            output_dir / "com_distance_convergence_summary.csv",
            index=False,
        )
    ...
    table1_like = make_table1_like_energy_components(ddg_df)
    if not table1_like.empty:
        table1_like.to_csv(output_dir / "table1_like_energy_components.csv", index=False)

    pos_summary = build_mutation_position_summary(ddg_df)
    if not pos_summary.empty:
        pos_summary.to_csv(output_dir / "mutation_position_summary.csv", index=False)
